...
func Eth_GetBlockTransactionCountByNumber(host string, token string, blockNumber string) (string, error) {

	//start := time.Now()
	//defer func() {
	//	log.Printf("Eth_GetBlockTransactionCountByNumber | duration=%v", time.Now().Sub(start))
	//}()
...
`
	...
	query = fmt.Sprintf(query, blockNumber)
	payload := strings.NewReader(query)
